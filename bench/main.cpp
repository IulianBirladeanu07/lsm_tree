...
std::vector<std::pair<std::string, int>> build_ops(
    const WorkloadDef& wl, const std::vector<int>& population, int num_ops, std::mt19937& rng)
{
    ...
    for (int i = 0; i < num_ops; ++i) {
        double r = coin(rng);
        if (r < wl.read_ratio)
            ops.push_back({"read",  population[pick(rng)]});
        else
            ops.push_back({"write", population[pick(rng)]});
    }
    return ops;
}
...
struct LsmBench {
    ...
    void insert(const std::string& key, const std::string& value) { tree->put(key, value); }
    void read(const std::string& key) { tree->get(key); }
};
...
struct SqliteHandle {
    ...
    void put(const std::string& key, const std::string& value) {
        ...
        sqlite3_prepare_v2(db,
            "INSERT OR REPLACE INTO kv(k,v) VALUES(?,?);", -1, &stmt, nullptr);
    ...
    void get(const std::string& key) {
        sqlite3_stmt* stmt;
        sqlite3_prepare_v2(db, "SELECT v FROM kv WHERE k=?;", -1, &stmt, nullptr);
        sqlite3_bind_text(stmt, 1, key.c_str(), -1, SQLITE_STATIC);
        sqlite3_step(stmt);
        sqlite3_finalize(stmt);
    }
...
void seed_lsm(lsm::LSMTree& tree, const std::vector<int>& population) {
    std::cout << "  seeding " << population.size() << " keys...\n";
...
int main() {
    ...
    for (const auto& wl : kWorkloads) {
        std::cout << "\n=== Workload " << wl.name
                  << " (read=" << wl.read_ratio * 100 << "% write=" << wl.write_ratio * 100 << "%) ===\n";
...

        {
            ...
            lsm::LSMOptions opts = tiered_opts;
    ...
    std::cout << "Run: python3 bench/plot.py\n";
    return 0;
}